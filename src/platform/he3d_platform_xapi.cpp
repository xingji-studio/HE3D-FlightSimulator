...
static int32_t g_windowCount = 0;
...
static void XapiDebugKey(const char *stage, UINT64 type, UINT64 raw, int32_t key, bool pressed)
{
   ...
   XapiAppendUnsigned(line, &pos, (int32_t)sizeof(line),
                      key < 0 ? (uint64_t)(-key) : (uint64_t)key);
...
static void XapiUnlockKeyQueue() { __sync_lock_release(&g_keyQueueLock); }

static Window *XapiFindWindow(HDLE handle)
{
   for (int32_t i = 0; i < g_windowCount; i++) {
      if (g_windows[i] && g_windows[i]->handle == handle) return g_windows[i];
   }
   return nullptr;
}

static bool XapiRegisterWindow(Window *window)
{
   if (!window || g_windowCount >= 16) return false;
   g_windows[g_windowCount++] = window;
   return true;
}

static void XapiUnregisterWindow(Window *window)
{
   for (int32_t i = 0; i < g_windowCount; i++) {
...
static void XapiDiscardWindowEvents(Window *window)
{
   XapiLockKeyQueue();
   int32_t cursor = g_keyQueueTail;
   while (cursor != g_keyQueueHead) {
      if (g_keyQueue[cursor].window == window) {
         XapiRemoveQueuedKeyEvent(cursor);
      } else {
         cursor = (cursor + 1) % XapiKeyQueueCapacity();
      }
   }
   XapiUnlockKeyQueue();
}
...
static void XapiQueueKeyEvent(Window *window, int32_t key, bool pressed)
{
   XapiLockKeyQueue();

   if (key >= 0 && key < 256) {
      if (window->keyDownState[key] == pressed) {
         XapiUnlockKeyQueue();
         return;
      }
      window->keyDownState[key] = pressed;
   }
   ...
   g_keyQueue[g_keyQueueHead].window  = window;
...
{
   Window *window = XapiFindWindow((HDLE)hData);
   if (!window) return;
   if (type == MSG_KEYDOWN || type == MSG_KEYUP) {
      ...
      XapiQueueKeyEvent(window, key, pressed);
...
static void *XapiAlloc(uint64_t size) { return xapi_AllocateMemory((UINT64)size); }
...
static Window *XapiCreateWindow(const WindowDesc *desc)
{
   ...
   for (int32_t i = 0; i < 256; i++) window->keyDownState[i] = false;
   XWINDOW xw;
...
static void XapiDestroyWindow(Window *window)
{
   ...
   XapiDiscardWindowEvents(window);
   XapiUnregisterWindow(window);
   xapi_CloseWindow(window->handle);
...
static void XapiPollEvents(Window *window)
{
   ...
   for (;;) {
      ...
      int32_t capacity = XapiKeyQueueCapacity();
      int32_t cursor   = g_keyQueueTail;
      while (cursor != g_keyQueueHead && g_keyQueue[cursor].window != window)
         cursor = (cursor + 1) % capacity;
      if (cursor == g_keyQueueHead) {
         XapiUnlockKeyQueue();
         break;
      }
      XapiKeyEvent event = g_keyQueue[cursor];
      if (cursor != g_keyQueueTail) g_keyQueue[cursor] = g_keyQueue[g_keyQueueTail];
      g_keyQueueTail = (g_keyQueueTail + 1) % capacity;
      XapiUnlockKeyQueue();
...
static bool XapiShouldClose(Window *window) { return window ? window->closeRequested : true; }
...
static void XapiSleepMilliseconds(uint64_t milliseconds) { xapi_Sleep((UINT64)milliseconds); }
...
static bool XapiEnsureScaleBuffer(Window *window)
{
   ...
   window->scaledPixels =
       new ColorA[(unsigned long)window->displayWidth * (unsigned long)window->displayHeight];
...
static const ColorA *XapiScaleToWindow(Window *window, int32_t width, int32_t height,
                                       const ColorA *pixels)
{
...
static const Platform g_xapiPlatform = {XapiAlloc,         XapiFree,
                                        XapiLoadFile,      XapiCloseFile,
                                        XapiCreateWindow,  XapiSetWindowTitle,
                                        XapiDestroyWindow, XapiSetKeyCallback,
                                        XapiPollEvents,    XapiShouldClose,
                                        XapiTimeSeconds,   XapiSleepMilliseconds,
                                        XapiPresent,       XapiGetApplicationBasePath};

const Platform *GetBuiltinPlatform() { return &g_xapiPlatform; }
