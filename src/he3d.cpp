...
static bool TriangleOutsideViewport(const float2 *ps, int width, int height) {
    if (ps[0].x < 0.0f && ps[1].x < 0.0f && ps[2].x < 0.0f)
        return true;
    if (ps[0].x >= (float)width && ps[1].x >= (float)width && ps[2].x >= (float)width)
        return true;
    if (ps[0].y < 0.0f && ps[1].y < 0.0f && ps[2].y < 0.0f)
        return true;
...
Mesh *Mesh::CreatePlane(float width, float depth) {
    ...
    float hz = depth * 0.5f;
    float3 vertices[6] = {
        {-hx, 0.0f, -hz}, {hx, 0.0f, hz}, {hx, 0.0f, -hz}, {-hx, 0.0f, -hz}, {-hx, 0.0f, hz}, {hx, 0.0f, hz}};
    float2 uvs[6] = {
...
void Renderer::DrawGameObject(const GameObject &obj, const Camera &cam, color3 color) {
    if (m_width <= 0 || m_height <= 0)
        return;
    if (!obj.mesh || obj.mesh->vertCount < 3)
        return;
    if (!obj.mesh->vertices || !obj.mesh->uvs)
        return;
    ...
    for (int i = 0; i < vc; i += 3, triIndex++) {
        ...
        if (float3::dot(float3::cross(ab, ac), vv[0]) >= 0)
            continue;
        ...
        for (int k = 1; k < clippedCount - 1; k++) {
            ...
            if (TriangleOutsideViewport(ps, m_width, m_height))
                continue;
            if (HE3D_ABS(ScreenTriangleArea(ps)) <= 0.0001f)
                continue;
...
static uint8_t TriangleSampleMask4(int x, int y,
...
bool Renderer::ApplyTaa(const ColorA *source) {
    ...
    for (int y = 0; y < m_height; y++) {
        for (int x = 0; x < m_width; x++) {
            ...
            int clampedColorDelta = HE3D_ABS((int)cur.r - histR) +
