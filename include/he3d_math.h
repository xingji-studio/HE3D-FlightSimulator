...
HE3D_ALWAYS_INLINE float fabsf(float x) { return x < 0.0f ? -x : x; }

HE3D_ALWAYS_INLINE int32_t abs(int32_t x) { return x < 0 ? -x : x; }
...
HE3D_ALWAYS_INLINE float roundf(float x) { return floorf(x + 0.5f); }
...
HE3D_ALWAYS_INLINE float cosf(float x) { return sinf(x + 1.5707963267948966f); }

HE3D_ALWAYS_INLINE float tanf(float x) { return sinf(x) / cosf(x); }
...
HE3D_ALWAYS_INLINE float atan2f(float y, float x)
{
   ...
   float at =
       z * (1.0f +
            z2 * (-0.333333283662796f +
                  z2 * (0.199993550777435f + z2 * (-0.142028367400169f + z2 * 0.10608633607626f))));
...
struct float3 {
   ...
   HE3D_MEMBER_INLINE float3 operator+(const float3 &v) const
   {
...
struct color3 {
   ...
   HE3D_MEMBER_INLINE color3(float _r = 0, float _g = 0, float _b = 0) : r(_r), g(_g), b(_b) {}

   HE3D_MEMBER_INLINE color3 operator+(const color3 &c) const
   {
...
struct AABB {
   ...
   HE3D_MEMBER_INLINE AABB(float3 _min = {0, 0, 0}, float3 _max = {0, 0, 0}) : min(_min), max(_max)
   {
   }
...
struct Ray {
   ...
   HE3D_MEMBER_INLINE Ray Normalized() const { return Ray(origin, direction.normalizeFast()); }

   HE3D_MEMBER_INLINE float3 At(float distance) const { return origin + direction * distance; }

   HE3D_MEMBER_INLINE bool IntersectSphere(float3 center, float radius,
   ...
   HE3D_MEMBER_INLINE bool IntersectPlane(float3 point, float3 normal,
                                          float *outDistance = nullptr) const
   {
   ...
   HE3D_MEMBER_INLINE bool IntersectTriangle(float3 v0, float3 v1, float3 v2,
                                             float *outDistance = nullptr, float *outU = nullptr,
   ...
   HE3D_MEMBER_INLINE bool IntersectAABB(const AABB &box, float *outNear = nullptr,
...
struct quat {
   ...
   HE3D_MEMBER_INLINE static quat FromEuler(float3 euler)
   {
      ...
      return {cy * cp * cr + sy * sp * sr, cy * sp * cr + sy * cp * sr, sy * cp * cr - cy * sp * sr,
              cy * cp * sr - sy * sp * cr};
   }
   ...
   HE3D_MEMBER_INLINE float3 rotate(const float3 &v) const
   {
      ...
      return {v.x + twoW * cross1.x + 2.0f * cross2.x, v.y + twoW * cross1.y + 2.0f * cross2.y,
